...
また、`.env` と `ow-custommatch-bot.db`（初回起動時に自動生成）は **実行ファイルと同じディレクトリ** に配置する前提です。
...
プレイヤー情報・VC 設定は SQLite（`ow-custommatch-bot.db`）に保存され、初回起動時に自動生成されます。
...
> 起動時に、`.env` を実行ファイルと同じディレクトリから読み込みます。`ow-custommatch-bot.db` は同ディレクトリに自動生成されます。
...
- `failed to initialize bot` が出る
  - 実行ファイル配置先に書き込み権限があるか確認（`ow-custommatch-bot.db` を生成できる必要があります）
- スラッシュコマンドが表示されない
