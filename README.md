...
- **利用者（運用者）**: Go 環境は不要です。GitHub Release から `ow-custommatch-bot.exe` を取得して起動してください。
- **開発者（ソースを変更して再ビルドする人）**: 開発者向け手順を参照（[docs/developer.md](docs/developer.md)）

`BOT_TOKEN` は初回起動時に入力すると保存されます。Windows では Credential Manager に保存され、SQLite データベースとログは `%LOCALAPPDATA%\ow-custommatch-bot\` 配下に保存されます。保存済みトークンを別の値に変えたい場合は、起動メニューの `トークンを上書きする` を使ってください。
...
4. `保存済みトークンを更新しました` と表示されたら、そのまま通常運用または動作確認用を選んで起動します。
...
- `BOT_TOKEN が保存されていません` が出る
  - `ow-custommatch-bot.exe` をダブルクリックまたはコンソールから起動し、初回入力を完了してください
- `別のトークンに変えたい`
  - 起動メニューの `トークンを上書きする` から新しい `BOT_TOKEN` を入力してください
- `BOT_TOKEN の設定に失敗しました` が出る
  - Windows Credential Manager に保存できなかった可能性があります。再起動して入力し直してください
- `failed to initialize bot` が出る
  - `%LOCALAPPDATA%\ow-custommatch-bot\` に書き込み権限があるか確認してください
- スラッシュコマンドが表示されない
