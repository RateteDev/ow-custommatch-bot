...
- `internal/bot/bot.go`:
  - `handleAssign`: フィールド名に `（平均スコア: N）` を付記
  - `teamAverageScore`: 平均スコア算出関数を追加
- `internal/bot/bot_test.go`: `teamAverageScore` のテスト追加（空チーム/2人チーム）
- `go test ./...` 全件パス ✅
- `go build ./...` 成功 ✅

## 次期改善事項

- Embed フッターに「スコアはランクに基づく内部値です」の補足追加を検討
